...
class MSCA(nn.Module):
    def __init__(self, channels=32, r=2):
        ...
        self.local_att = nn.Sequential(
            ...
            nn.Conv2d(
                out_channels, channels, kernel_size=1, stride=1, padding=0, bias=False
            ),
...
class ACFMSimple(ACFM):
    def __init__(self, channel=32):
        super(ACFMSimple, self).__init__()
...
class MBR(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, upsample=None, **kwargs):
        ...
        self.conv3 = nn.Conv2d(
            inplanes, inplanes, kernel_size=5, stride=1, padding=2, bias=False
        )
        ...
        if upsample is not None and stride != 1:
            ...
            self.conv4 = nn.Conv2d(
                inplanes, planes, kernel_size=5, stride=stride, padding=2, bias=False
            )
...
class C2FNet(nn.Module):
    def __init__(self, channel=32):
        ...
        self.upconv1 = BasicConv2d(
            channel, channel, kernel_size=3, stride=1, padding=1, relu=True
        )
        self.upconv0 = BasicConv2d(
            channel, channel, kernel_size=3, stride=1, padding=1, relu=True
        )
        self.Unet = UNet()

        self.conv1 = BasicConv2d(
            channel * 2, channel, kernel_size=3, stride=1, padding=1, relu=True
        )
        ...
        self.uconv1 = BasicConv2d(
            channel * 2, channel, kernel_size=3, stride=1, padding=1, relu=True
        )
        self.uconv2 = BasicConv2d(
            channel * 2, channel // 2, kernel_size=3, stride=1, padding=1, relu=True
        )
...
class BasicC2FNet(C2FNet):
    def __init__(self, channel=32):
        super(BasicC2FNet, self).__init__()
    ...
    def forward(self, x):
        ...
        x2_1 = self.rfb2_1(x2)  # channel -> 32
        x3_1 = self.upsample(self.rfb3_1(x3), scale_factor=2)  # channel -> 32
        x4_1 = self.upsample(self.rfb4_1(x4), scale_factor=4)  # channel -> 32

        x234 = torch.cat((x2_1, x3_1, x4_1), 1)
        s2 = self.classifier1(x234)
        ...
        s0 = self.classifier2(self.upsample(x210, scale_factor=4))
        s2 = F.interpolate(s2, scale_factor=8, mode="bilinear", align_corners=False)

        return s2, s0


class BasicCIMC2FNet(C2FNet):
    def __init__(self, channel=32):
        super(BasicCIMC2FNet, self).__init__()
        self.classifier1 = nn.Conv2d(channel * 3, 1, 1)
        self.upsample = cus_sample
...
class BasicACFMC2FNet(C2FNet):
    def __init__(self, channel=32):
        super(BasicACFMC2FNet, self).__init__()
        self.classifier1 = nn.Conv2d(channel, 1, 1)
        self.upsample = cus_sample
    ...
    def forward(self, x):
        ...
        x43 = self.acfm3(x3_1, x4_1)  # bs, 32, 22, 22

        x432 = self.acfm2(x2_1, x43)  # bs, 32, 44, 44

        s2 = self.classifier1(x432)
...
class BasicDGCMC2FNet(C2FNet):
    def __init__(self, channel=32):
        super(BasicDGCMC2FNet, self).__init__()
        self.classifier1 = nn.Conv2d(channel, 1, 1)
        self.upsample = cus_sample
    ...
    def forward(self, x):
        ...
        x43 = x3_1 + self.upsample(x4_1, scale_factor=2)
        out43 = self.upconv3(self.dgcm3(x43) + x43)

        x432 = x2_1 + self.upsample(out43, scale_factor=2)
        out432 = self.upconv2(self.dgcm2(x432) + x432)
...
class BasicACFMDGCMC2FNet(C2FNet):
    def __init__(self, channel=32):
        super(BasicACFMDGCMC2FNet, self).__init__()
        self.classifier1 = nn.Conv2d(channel, 1, 1)
        self.upsample = cus_sample
...
class C2FNetWOMSCA(C2FNet):
    def __init__(self, channel=32):
        super(C2FNetWOMSCA, self).__init__()

        self.acfm3 = ACFMSimple()
        self.acfm2 = ACFMSimple()
        self.dgcm3 = DGCMSimple()
        self.dgcm2 = DGCMSimple()

        self.acfm1 = ACFMSimple()
        self.acfm0 = ACFMSimple()
        self.dgcm1 = DGCMSimple()
        self.dgcm0 = DGCMSimple()
